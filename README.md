# Jarvis AI Center (SituationDeck)

Личный AI-центр и Day OS: `FastAPI + Telegram(aiogram) + SQLite + локальная Ollama`.
Проект развивается как локальный "операционный центр дня" с веб-дашбордом и core-маршрутом:
`today -> todo -> focus -> checkin -> week/review`.
...
│  ├─ tasks.py               # Core-логика задач
│  ├─ subs.py                # Core-логика подписок
│  └─ day_os.py              # Core day-overview
├─ app/
│  └─ api.py                 # FastAPI transport поверх core
├─ handlers/
...
Короткие архитектурные карты:
- `ARCHITECTURE.md` — обзор системы простым языком.
- `CORE_ROUTE.md` — линейный маршрут ядра (transport -> coordinator -> core -> db).
...
Единый запуск Web API + Dashboard (оба ОС):
```bash
...
- подробный runbook: `RUNBOOK_DEPLOY_V1.md`
...
Dashboard (веб-прототип Personal Command Center):
```text
GET /dashboard
GET /dashboard/data?user_id=<id>&debug=0|1&ai=0|1
POST /tasks        # text + due_date + remind_at
POST /subs         # name + amount + next_date + period
GET /signals?user_id=<id>&limit=20
```
Флаги:
- `API_DEBUG_EVENTS=1` — разрешить возврат `events` в API при `debug=1`
- `DASHBOARD_ALLOW_PUBLIC=0` — запретить публичный доступ к API/dashboard (только trusted CIDR)
- `DASHBOARD_TRUSTED_NETS=...` — список доверенных сетей (CIDR)
- `API_DEBUG_EVENTS_REMOTE=0` — не выдавать debug events удалённым клиентам
- `DASHBOARD_AI_BRIEF=1` — включить LLM-brief в дашборде (`ai=1`)
- `DASHBOARD_AI_EVERYWHERE=1` — применить LLM-полиш ко всем ключевым панелям (миссия/рост/погода/тренировка/рынок)
- `ENABLE_TASK_REMINDERS=true` — Telegram-напоминания по задачам с `remind_at`
- `TASK_REMINDER_INTERVAL_SECONDS=45` — интервал проверки reminder-очереди
- `INGEST_IMAP_ENABLED=1` — включить ingestion сигналов из IMAP
- `INGEST_LOCAL_ENABLED=1` + `INGEST_LOCAL_PATHS=...` — включить ingestion сигналов из локальных файлов
- `RAG_ENABLED=1` — включить retrieval по личным данным
- `RAG_REQUIRE_CITATIONS=1` — требовать цитаты источников для personal-запросов
...
## 14) GitHub Sync (быстрый чек-лист)
```bash
# 1) обновить локальную ветку
git pull --rebase

# 2) проверить секреты и тесты
python scripts/check_secrets.py
python -m unittest discover -s tests -p "test_*.py"

# 3) сохранить изменения
git add .
git commit -m "update: dashboard + day os improvements"

# 4) отправить в GitHub
git push origin main
```

Если пуш отклонён по конфликтам:
```bash
